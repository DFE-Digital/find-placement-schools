class DfeSignInUser
  ...
  def self.load_from_session(session)
    ...
    return unless dfe_sign_in_session["last_signed_in_at"]

    return if dfe_sign_in_session.fetch("last_signed_in_at") < SESSION_TIMEOUT.ago

    dfe_sign_in_session["last_signed_in_at"] = Time.current

    new(
      email: dfe_sign_in_session["email_address"],
