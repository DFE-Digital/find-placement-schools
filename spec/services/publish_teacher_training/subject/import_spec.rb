  ...
  describe ".call" do
    context "when the response contains only valid subject data" do
      it "creates a subject for each valid subject in the API response" do
        ...
        }.to change(PlacementSubject.primary, :count).by(2).and change(
        ...
        expect(PlacementSubject.secondary.pluck(:name)).to contain_exactly("Art and design", "Science", "French", "Modern Languages")

        modern_languages = PlacementSubject.find_by(name: "Modern Languages")
      ...
      context "when a subject already exists" do
        it "only creates a subject for each not pre-existing subject" do
          ...
          expect {
            described_class.call
          }.to change(PlacementSubject, :count).by(5)
        end
      end
    end
    ...
    context "when the response contains an invalid subject data" do
      ...
      it "only creates a the valid subject" do
        expect { described_class.call }.to change(PlacementSubject.primary, :count).by(1)
