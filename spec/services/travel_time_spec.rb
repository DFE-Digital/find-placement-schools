...
RSpec.describe TravelTime do
  ...
  let(:school_a) { create(:school, name: "Abbey Academy", longitude: 1.2, latitude: 0.2) }
  let(:school_b) { create(:school, name: "Buxton Boarding School", longitude: 1.2, latitude: 0.2) }
  let(:school_c) { create(:school, name: "Canterbury College", longitude: 1.2, latitude: 0.2) }
  let(:destinations) { School.order(:name) }
  ...
  describe "#call" do
    ...
    it "caches the travel time data" do
      ...
      expect(Rails.cache.exist?("travel_time_#{fingerprint}")).to be true
    end
  end
end
