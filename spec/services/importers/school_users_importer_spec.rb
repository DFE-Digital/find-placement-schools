...
RSpec.describe Importers::SchoolUsersImporter do
  ...
  describe "#call" do
    let(:csv_path) { "spec/fixtures/importers/school_users.csv" }

    context "when the data is valid" do
      let(:school) { create(:school, urn: "110048") }
      ...
      it "does not send an invite to the user if they are already invited" do
        ...
        described_class.call(csv_path)
        described_class.call(csv_path)
        expect(Users::Invite).to have_received(:call).with(user: an_instance_of(User), organisation: school).once
      end

      it "sends an invite to the user" do
        school
        described_class.call(csv_path)
        expect(Users::Invite).to have_received(:call).with(user: an_instance_of(User), organisation: school)
      end

      context "when a user is in two schools" do
        let!(:school2) { create(:school, urn: "114433") }
        ...
        it "associates the user to both schools" do
          ...
          expect(user.schools).to contain_exactly(school, school2)
        end
      end
    end

    context "when the user details are not valid" do
      let!(:school) { create(:school, urn: "110063") }

      before do
        create(:school, urn: "110048")
        create(:school, urn: "114433")
      end
