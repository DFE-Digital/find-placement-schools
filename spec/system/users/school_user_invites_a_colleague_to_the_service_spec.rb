...
RSpec.describe "School user invites a colleague to the service", type: :system do
  scenario do
  ...
  def then_i_see_the_confirm_user_details_page
    ...
    expect(page).to have_summary_list_row("Email address", "hermione.granger@hogwarts.com", "Change")
    expect(page).to have_warning_text("Hermione Granger will be sent an email to tell them you’ve added them to Hogwarts.")
  ...
  def then_i_see_the_users_page_with_a_success_message
    ...
    expect(page).to have_success_banner(
      "User added",
      "Hermione is now able to use this service on behalf of Hogwarts"
    )
  end
end
