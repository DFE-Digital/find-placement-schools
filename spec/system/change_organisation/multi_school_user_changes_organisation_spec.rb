...
RSpec.describe "Multi school user changes organisation", type: :system do
  ...
  def then_i_see_the_placement_preferences_form_page_for_hogwarts_school
    expect(page).to have_title("Can your school offer placements for trainee teachers in the #{@current_academic_year_name} academic year? - Find placement schools")
  ...
  def then_i_see_the_placement_preferences_form_page_for_springfield_school
    expect(page).to have_title("Can your school offer placements for trainee teachers in the #{@current_academic_year_name} academic year? - Find placement schools")
    expect(page).to have_important_banner("You have changed your organisation to Springfield")
    expect(page).to have_element(
      :legend,
      text: "Can your school offer placements for trainee teachers in the #{@current_academic_year_name} academic year?",
      class: "govuk-fieldset__legend",
