...
RSpec.describe OnboardUsersWizard::SchoolUploadStep, type: :model do
        ...
        context "when the csv_upload is a CSV file" do
          ...
          let(:valid_file) do
            ActionDispatch::Http::UploadedFile.new({
              ...
              type: "text/csv",
              tempfile: File.open(
                "spec/fixtures/importers/school_users.csv",
              )
            })
          end
          let(:london_school) { create(:school, name: "London School", urn: 111_111) }
          let(:guildford_school) { create(:school, name: "Guildford School", urn: 222_222) }
        ...
        context "when the csv content is missing valid headers" do
          ...
          it "returns errors for missing headers" do
            ...
            expect(step.errors.messages[:csv_upload]).to include(
              "Your file needs a column called ‘urn’, ‘email_address’, ‘first_name’, and ‘last_name’.",
            )
            expect(step.errors.messages[:csv_upload]).to include(
  ...
  describe "#csv_inputs_valid?" do
    ...
    context "when the csv_content contains valid email_address, school urn, and all necessary valid attributes" do
      let(:csv_content) do
        "school_name,urn,first_name,last_name,email_address\r\n" \
        "London School,111111,John,Smith,john_smith@example.com\r\n" \
        ",,,,"
  ...
  describe "#process_csv" do
    ...
    it "reads a given CSV and assigns the content to the csv_content attribute" do
      expect(step.csv_content).to eq(
        ...
        "110048,Jeff,Barnes,j.barnes@example.com\n" \
        "114433,Jeff,Barnes,j.barnes@example.com\n" \
        "114433,James,Hill,j.hill@example.com\n" \
        "110063,Sarah,Smith,\n",
      )
    end
  end
  ...
  describe "#csv" do
    ...
    it "converts the csv content into a CSV record" do
      ...
      expect(csv.headers).to match_array(
        %w[school_name urn first_name last_name email_address],
      )
