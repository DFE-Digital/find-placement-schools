module GovukComponentMatchers
  ...
  matcher :have_summary_list_row do |expected_key, expected_value: nil, expected_action: nil|
    match do |page|
      key = page.find(".govuk-summary-list__key", text: expected_key)
      row = key.find(:xpath, "..", class: "govuk-summary-list__row")
      row.find(".govuk-summary-list__value", text: expected_value) if expected_value
      row.find(".govuk-summary-list__actions a", text: expected_action) if expected_action
      true
  ...
  def service_navigation
    page.find(".govuk-service-navigation__container")
  end

  # Usage: expect(primary_navigation).to have_current_item("Schools")
  matcher :have_current_item do |text|
    match do |page|
      page.find(".govuk-service-navigation__link[aria-current='page']", text:)

      # assert there is only one current nav item
      page.all(".govuk-service-navigation__link[aria-current='page']", count: 1)
      true
    rescue Capybara::ElementNotFound
      false
    end
